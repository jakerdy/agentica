...
3. Обносить версии в CLI пакетах и прочих константах в коде.
4. Дописать все изменения из черновика changelog в `CHANGELOG.md` (если есть).
...
7. Закоммитить изменения с сообщением `chore(release): prepare vX.Y.Z`.
8. Создать тег `vX.Y.Z` и отправить его в origin.
9. Написать финальную команду для публикации релиза и дать краткие инструкции по её запуску.
