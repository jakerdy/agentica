...
Задавай вопросы через `ask_questions`, если:
...
## Сценарий 1: [Название]
```
[Точка входа] → [Обработка 1] → [Обработка 2] → [Результат]
```
...
```
[ASCII или Mermaid диаграмма, показывающая последовательность вызовов]

Trigger → Handler → Validator → Processor → Storage
            ↓                       ↓
        Logger                  EventEmitter
```
