...
## Структура Agentica в репозитории
...
│   ├── prompts/                # Промпты Agentica (init/create/change/...)
...
│   └── status.md               # Текущий статус интеграции Agentica
...
│   │   │   ├── architecture/   # Архитектурные документы пакета (AR-XXXX ...)
│   │   │   ├── changes/        # Change-спеки пакета (CH-XXXX ...)
│   │   │   ├── features/       # Feature-спеки пакета (FT-XXXX ...)
│   │   │   ├── product.md      # Продуктовый контекст пакета
│   │   │   ├── structure.md    # Структура именно этого пакета
│   │   │   ├── tech.md         # Технические правила и стек пакета
│   │   │   └── status.md       # Текущий статус интеграции Agentica
...
### Правила работы Agentica с файлами

- Все артефакты Agentica создаются и обновляются только внутри `.agentica/`.
- Root проекта содержит: шаблоны, промпты и общий контекст (`templates/`, `prompts/`, `product.md`, `structure.md`, `tech.md`, `status.md`).
- Package root содержит: фичи, изменения, архитектуру и общий контекст (`features/`, `changes/`, `architecture/`, `product.md`, `structure.md`, `tech.md`, `status.md`).
- Для scope всей монорепы используется `./.agentica/*`.
- Для scope конкретного пакета используется `./packages/<package-name>/.agentica/*`.
- Agentica не изменяет `README.md` автоматически в рамках инициализации, если это явно не запрошено.
- Текущий этап интеграции всегда фиксируется в `.agentica/status.md` (repo-level или package-level).
- Допустимые формулировки статуса:
  - `Текущий статус интеграции: проект создан`
  - `Текущий статус интеграции: интеграция в процессе по сценарию <N>`
  - `Текущий статус интеграции: интеграция завершена успешно`

#### Что лежит в `.agentica/` пакета

- `product.md`
  - Продуктовый контекст пакета: кто пользователь, какую проблему решает модуль,
    какие ограничения и критерии ценности.

- `structure.md`
  - Актуальная структура директорий и границ ответственности конкретного пакета.

- `tech.md`
  - Технический стек, стандарты кода, правила тестирования/сборки/валидации для пакета.

- `architecture/`
  - Хранит архитектурные документы пакета.
  - Формат: `AR-XXXX - <Название>.md`.
  - Назначение: фиксировать решения по модульным границам, интерфейсам и компромиссам.

- `changes/`
  - Хранит документы изменений уже существующей функциональности.
  - Формат: `CH-XXXX - <Название>.md`.
  - Назначение: миграции, совместимость, риски, rollout-план.

- `features/`
  - Хранит каталоги feature-спек.
  - Формат: `FT-XXXX - <Название>/`.
  - Внутри каждой фичи обязательно:
    - `product.md` - описание нового функционала с продкутовой точки зрения (дизайн-док + UX)
    - `tech.md` - описание нового функционала с технической точки зрения (архитектура + тех-задание)
    - `validation.md` - чеклист проверки на соответствие качественным критериям
    - `tasks.md` - TODO список по имплементации
  - Назначение: единый пакет артефактов для цикла create → iterate → tasks → implement → validate.

#### Что лежит в `.agentica/` репозитория (корень)

- `prompts/`
  - Хранит рабочие промпты Agentica на уровне репозитория.
  - Минимально ожидаемые файлы:
    ...
    - `readme.prompt.md`
    - `refactor.prompt.md`
  - Назначение: пакетная адаптация поведения агента без влияния на другие пакеты монорепы.

- `templates/`
  - Хранит набор шаблонов для инициализации новых спецификаций

- `status.md`
  - Точка истины по текущему этапу внедрения Agentica в этом пакете.
  - Допустимые статусы:
    - `Текущий статус интеграции: интеграция завершена успешно`
    - `Текущий статус интеграции: интеграция в процессе по сценарию <N>`

### Правила инициализации
Если директория или файл из этого списка отсутствует, `agentica.init` должен создать их в scope текущего пакета, не выходя за пределы `.agentica/`.

Если проект не является монорепой, вышеописанные структуры объединяются.
