...
5. Изменения требуют **полной переписки архитектуры** — это не change, это новая `rnd` + `refactor`.
...
5. Список существующих файлов в `features/` и `research/` (возможно, изменение связано с существующей спекой).
...
- Если есть `RnD-XXXX` — прочитай архитектурные решения.
...
2. Предложи план согласования (создать `RnD-XXXX` для изменения архитектуры).
