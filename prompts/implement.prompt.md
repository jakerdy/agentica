...
2. Файл `tasks.md` **пустой или отсутствует** (используй `agentica.tasks`).
3. Git ветка **не соответствует** паттерну `<scope-id>/FT-XXXX-<slug>`.
4. Запрос требует **изменения спецификации** (используй `agentica.iterate`).
5. Запрос требует **рефакторинга** без изменения функциональности (используй `agentica.refactor`).
...
1. `product.md` — **ОБЯЗАТЕЛЬНО**. Если пустой → STOP (используй `agentica.create`).
2. `tech.md` — **ОБЯЗАТЕЛЬНО**. Если пустой → STOP (используй `agentica.create`).
3. `tasks.md` — **ОБЯЗАТЕЛЬНО**. Если пустой → STOP (используй `agentica.tasks`).
4. `validation.md` — **ОБЯЗАТЕЛЬНО**. Если пустой → STOP (используй `agentica.tasks`).
...
- Если всё ещё неясно — задай вопрос пользователю через `ask_questions`
