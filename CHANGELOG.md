...
### Добавлено
- Новая CLI-команда `agentica safety-check` для детерминированной проверки staged-изменений на артефакты, секреты, большие файлы и потенциально неуместные фразы

### Изменено
- Workflow `/agentica.commit` переведён на вызов `agentica safety-check` вместо агентных эвристик внутри prompt

### Исправлено
- Убрано ложноположительное срабатывание NSFW-проверки на собственные regex-правила внутри `safety-check`

### Docs / Build
- Обновлён version badge в README до `0.1.0`
- Синхронизированы версии пакета и CLI до `0.1.0`

## [0.0.11] - 2026-02-24

### Изменено
- Команда `/agentica.architect` переименована в `/agentica.rnd`, промпт полностью переработан под R&D-исследования (оценка целесообразности, архитектурные советы, сравнение библиотек)
- Директория `.agentica/architecture/` переименована в `.agentica/research/`, префикс `AR-` заменён на `RnD-` во всех шаблонах, стеках, промптах и документации

### Docs / Build
- Обновлён version badge в README до `0.0.11`
- Исправлена захардкоженная версия в `src/cli.ts` (была `0.0.8`)
