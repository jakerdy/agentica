...
*Условие:* Проект был проинициализирован только что, и в нём ещё не настроены промпты под целевой стек.
1. Работай только в root scope (`./.agentica/prompts/`).
2. Определи стек технологий из `tech.md` и текущих инструментов в проекте.
3. Обнови все промпты в `prompts/` под эти технологии. Базово они написаны по использование в среде typescript + bun. Но если там, например, Python + pytest + flake8, то нужно поправить промпты так, чтобы они были релевантны этому стеку. Например, в промпте для тестов указать pytest вместо bun test и т.д.
  - Запусти сематический поиск на тему "запуска интсрументов", чтобы найти релевантные команды и поправить их во всех остальных промптах для соответствия тех-стеку проекта.
  - Найди все упоминания bun и tsc через regex и отредактируй их под релевантные инструменты.
  - Отдельно посмотри на refactor.prompt.md и validate.prompt.md, там могут быть специфичные команды для линтинга и тестов, которые нужно адаптировать.
4. Статус: `настройка промптов`.
