...
[![Version](https://img.shields.io/badge/version-0.2.0-blue)](https://github.com/jakerdy/agentica/releases/tag/v0.2.0)
...
Agentica CLI поддерживает два формата запуска `init` и интерактивный мастер:
- **Интерактивный:** `agentica init`
...
```bash
# Пошаговая интерактивная инициализация
agentica init
...
В интерактивном режиме CLI последовательно предложит:
- выбрать один из доступных стеков;
- указать директорию проекта или оставить текущую;
- определить, какие секции войдут в `AGENTS.md`.
...
| `commit`    | `--skip-checks` | Коммит с вызовом CLI-проверки безопасности             |
...
Используйте, когда нужно автоматически собрать staged-изменения, быстро прогнать CLI safety-check и создать качественное сообщение коммита.

Или просто используйте его всегда, когда собираетесь коммитить. Теперь safety-аудит делает не агент, а отдельная CLI-команда с фиксированными эвристиками, поэтому это заметно быстрее и стабильнее:
...
```
*Агент добавит изменения в stage, запустит `agentica safety-check`, покажет найденные риски, при необходимости предложит осознанно обойти их через `--skip-checks`, сформирует сообщение в формате `<change_type>: <Short Change Description>` + детальное описание и выполнит `git commit`.*

Если нужно просто отдельно проверить staged-изменения без запуска агента и без коммита:

```bash
agentica safety-check
```

Для автоматизаций доступен JSON-режим:

```bash
agentica safety-check --json
```
